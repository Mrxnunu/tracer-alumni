...
<div class="bg-white shadow-md rounded-lg mb-6 dark:bg-slate-900">
  ...
  <div class="p-6 text-center">
    ...
    <p class="text-gray-600 mb-4">
      <small>By. <a href="/blog?authors={{ $posts[0]->author->username ?? 'Tidak ada author' }}" class="text-amber-500 hover:underline">{{ $posts[0]->author->name }}</a> in <a href="/blog?category={{ $posts[0]->category->slug }}" class="text-amber-500 hover:underline">{{ $posts[0]->category->name }}</a> {{ $posts[0]->created_at->diffForHumans() }}</small>
    </p>
