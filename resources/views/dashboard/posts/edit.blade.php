        ...
        <nav class="flex px-5 py-3 text-gray-700 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-800 dark:border-gray-700" aria-label="Breadcrumb">
          <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
              ...
              <div class="flex items-center">
                  ...
                  <a href="/dashboard/posts" class="ms-1 text-sm font-medium text-amber-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">Artikel</a>
              </div>
              </li>
          </ol>
      </nav>
        <div class="py-5">
            <form method="post" action="/dashboard/posts/{{ $post->id }}" class="w-11/12 m-auto" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <h2 class="text-2xl leading-tight font-medium mb-2">Edit Artikel</h2>
                <div class="mb-5">
                    <label for="title" class="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-500">Title</label>
                    <input type="text" id="title" name="title" class="bg-gray-50 border border-gray-500 text-gray-900 dark:text-gray-400 placeholder-gray-700 dark:placeholder-gray-500 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-500 @error('title') is-invalid @enderror" required autofocus value="{{ $post->title }}">
                    @error('title')
                ...
                <div class="mb-5">
                    ...
                    <input type="text" id="slug" name="slug" class="bg-gray-50 border border-gray-500 text-gray-900 dark:text-gray-400 placeholder-gray-700 dark:placeholder-gray-500 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-500 @error('slug') is-invalid @enderror" required autofocus value="{{ $post->slug }}">
                    @error('slug')
                    <p id="title" class="mt-2 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mb-5">
                    <label for="category_id" class="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-500">Select Category</label>
                ...
                <div class="mb-5">
                    ...
                    <img class="img-preview object-cover mb-3 sm:w-1/3" src="{{ asset($post->image) }}" alt="Post Image">
